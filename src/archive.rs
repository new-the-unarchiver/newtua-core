...
impl<'a> SinkWriter<'a> {
    ...
    /// Сам приёмник — тому, кто держит писаря и одновременно ведёт границы
    /// записей.
    ///
    /// Нужен обработчику, чей обход требует **владеемого** писаря, пережившего
    /// вызов (`format/rar.rs`): приёмник у него один на проход, а `begin`/`end`
    /// зовутся между телами. Прочим хватает `SinkWriter` как обычного `Write`.
    pub(crate) fn sink(&mut self) -> &mut dyn EntrySink {
        self.sink
    }
