...
use super::{Error, Result, huffman};
...
impl Unpack50Decoder {
    ...
    // VENDORED.md). Оба совета clippy здесь косметические, а цена ошибки —
    // скорость, которую мерили парными прогонами: `&mut Vec` вместо среза
    // держит вызывающего у одного буфера, а ручной счётчик в перекрытом
    // копировании сохраняет порядок байт, на котором стоит весь LZ. Менять —
    // только с замером в руках, не по подсказке.
...
mod tests {
    ...
    fn copies_lz_matches_with_overlap() {
        ...
        ensure_copy_room(&mut output, pos, 8);

        decoder
            .copy_match(&mut output, &mut pos, 2, 6, 8, DEFAULT_DICTIONARY_SIZE)
            .unwrap();
    ...
    fn rejects_invalid_match_copy() {
        ...
        let mut output = b"AB".to_vec();
        let mut pos = output.len();
        ensure_copy_room(&mut output, pos, 3);

        assert_eq!(
            decoder.copy_match(&mut output, &mut pos, 3, 1, 3, DEFAULT_DICTIONARY_SIZE),
            Err(Error::InvalidData("RAR 5 match distance exceeds window"))
        );
        assert_eq!(
            decoder.copy_match(&mut output, &mut pos, 1, 2, 3, DEFAULT_DICTIONARY_SIZE),
