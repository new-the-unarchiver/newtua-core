/// The checksum a CAB data block carries — a folded XOR over the block, not a
/// CRC. Verified while a block is loaded; a block whose stored value is zero
/// carries none.
#[derive(Default)]
pub struct Checksum {
    value: u32,
...
impl Checksum {
    ...
    pub fn value(&self) -> u32 {
        match self.remainder_shift {
            ...
            16 => self.value ^ (self.remainder >> 8) ^ ((self.remainder & 0xff) << 8),
            24 => {
                self.value
                    ^ (self.remainder >> 16)
