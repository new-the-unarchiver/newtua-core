...
///
/// G14, названный внутри тикета 37 §5. Проход по архиву обрывался на первой же
/// неудаче и, начатый заново, спотыкался о ту же запись, — так весь остаток
/// набора объявлялся потерянным. Вне сплошного архива соседи от испорченной
/// записи не зависят. На этом образце было три целых файла из шести, стало
/// пять — столько же, сколько достаёт `unrar`; последний шаг (разрезанную
/// границей тома запись) стережёт тест ниже.
///
...
fn a_damaged_entry_does_not_take_its_neighbours_with_it() {
    ...
    // А если кто-то всё же не дошёл, ему полагается правда о причине, а не
    // «заголовки кончились»: заголовки на месте, прекратился обход.
...
/// Разрезанная границей тома запись тоже переживает порчу соседа.
///
/// Остаток G14. Поодиночке такую запись не прочитать — её склеивает только
/// обход набора, — а обход прекращался на первой же неудаче. `unrar` на этом
/// образце достаёт пять файлов из шести, и это та самая пятая: `t/f2.bin`
/// начинается во втором томе и кончается в третьем, испорчено же тело
/// `t/f1.bin`, лежащее перед ней.
///
/// Судья — не мы: байты сверяются с тем потоком, из которого фикстура собрана,
/// а число уцелевших — с `unrar` на этом же наборе.
#[test]
fn a_split_entry_survives_a_damaged_neighbour() {
    let (_dir, mut ar) = open_torn(Some(500));
    let at = index_of(ar.as_mut(), &numbered("t", "f", 6));
    let sink = read_all(ar.as_mut());
    ...
    assert_eq!(
        sink.body(at[2]),
        &bodies[2][..],
        "разрезанная t/f2.bin обязана дойти целой"
    );
    // Пять из шести целыми, отказ ровно у испорченной — как у `unrar`.
    for i in [0usize, 2, 3, 4, 5] {
        assert_eq!(sink.body(at[i]), &bodies[i][..], "запись t/f{i}.bin");
    }
    assert_eq!(
        sink.failed,
        vec![at[1]],
        "отказать имеет право только испорченная запись"
    );
}

/// Та же разрезанная запись, запрошенная одна: испорченного соседа никто не
/// просил, и его беда не должна её отменять.
///
/// Отдельный случай, а не повторение предыдущего: в проходе по всему архиву
/// испорченная запись **запрошена**, её отказ уходит приёмнику и обход идёт
/// дальше. Здесь она не запрошена вовсе, приписать отказ некому — а идти дальше
/// всё равно надо, иначе распаковка одного файла из повреждённого набора
/// оказывается слабее распаковки всех.
#[test]
fn a_lone_split_entry_is_read_past_a_damaged_stranger() {
    let (_dir, mut ar) = open_torn(Some(500));
    let at = index_of(ar.as_mut(), &numbered("t", "f", 6));

    let mut body = Vec::new();
    ar.read_entry(at[2], &mut body).unwrap();

    assert_eq!(body, torn_bodies()[2], "разрезанная t/f2.bin");
}

// solidtorn.rar: четыре сжимаемых файла в **сплошном** архиве:
//   rar a -r -s -m5 -ep1 solid.rar s
// Фикстура на диске целая; портится копия в памяти, как и у `torn`.
const SOLID_TORN: &[u8] = include_bytes!("../fixtures/solidtorn.rar");

/// В сплошном архиве обход после испорченной записи **не** продолжается.
///
/// Граница правки G14, и она проведена не из осторожности вообще, а по замеру:
/// на этом же образце `unrar` после порчи в теле первой записи объявляет
/// испорченными **все четыре** — в сплошном архиве следующая запись живёт в
/// окне, накопленном предыдущими. Продолжать там значит подавать негодные
/// байты, надеясь на CRC. Поэтому соседям говорится, что обход до них не дошёл,
/// и это правда.
#[test]
fn a_solid_archive_does_not_walk_past_damage() {
    let mut bytes = SOLID_TORN.to_vec();
    bytes[100] ^= 0xff;
    let (_tmp, mut ar) = open_fixture(&bytes, &OpenOptions::default());
    let at = index_of(ar.as_mut(), &numbered("s", "g", 4));
    let sink = read_all(ar.as_mut());

    assert!(
        sink.failed.contains(&at[0]),
        "испорченная запись обязана отказать, отказали {:?}",
        sink.failed
    );
    for &idx in &at[1..] {
        assert!(
            sink.reason(idx).contains("обход архива прервался"),
            "сосед по сплошному архиву читаться не должен: {}",
            sink.reason(idx)
        );
    }
}
