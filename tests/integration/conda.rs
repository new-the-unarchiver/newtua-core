...
fn conda_reports_conda_and_lists_inner_files() {
    ...
    assert!(
        names
            .iter()
            .any(|n| n == Path::new("lib/python3.12/site-packages/foo/__init__.py"))
    );
    assert!(names.iter().any(|n| n == Path::new("info/index.json")));
    assert!(
        !names
            .iter()
            .any(|n| n.to_string_lossy().ends_with(".tar.zst"))
    );
    assert!(!names.iter().any(|n| n == Path::new("metadata.json")));
}
